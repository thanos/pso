...
#
#   $Id: parser.py,v 1.15 2002/06/03 15:56:37 thanos Exp $
#
...
class PSOParts:
    def __init__(self, parent=None, name ='', className='', start=0, end=0, text=''):
        self.setup(parent, name, className, start, end, text)
        self.children=[]
        
    def setup(self, parent, name, className, start, end, text):
        self.parent = parent
        self.name  = name
...
def psoSetup(object, parent, name, className, start, end, text):
        if isinstance(object, PSO):
            object.psoParts.setup(parent, name, className, start, end, text)
        else:
            object.psoParts = PSOParts(parent, name, className, start, end, text)
...
class PSO:    
    ...
    def preProcess(self, renderer=None): return ''
...
class Parser:
    ...
    def clear(self):
            self.__class__.sTagsAccepted={}
            self.__class__.sTagsRejected={}
    ...
    def getPSO(self, parent, args):
        ...
        if  self.sTagsAccepted.has_key(className):
                renderer= self.sTagsAccepted[className]
                #return renderer
        else:
        ...
        if renderer:
            ...
            if objectType is ClassType:
                ...
                psoSetup(renderer, parent, attrs['tagname'], className, start, end, text)
